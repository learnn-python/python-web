...
[进入官网](https://dev.mysql.com/downloads/mysql/)，下载mac版本的安装包，下载完成后一路安装下去即可，`注意安装过程中告诉我们生成一个root账户的密码，这个要记住，否则重设密码较麻烦`。

如果官网安装太慢，可以先去官网找到下载链接，然后使用迅雷工具安装，速度会快很多。

- 启动mysql

mac环境下，找到系统偏好设置，找到mysql启用。

- 进入mysql操作界面

在终端，输入`PATH="$PATH":/usr/local/mysql/bin`，然后输入`mysql -u root -p`，会提示输入密码，按要求输入完成后则进入sql操作界面。

记住，每一条mysql语句是以分号结束的。

- 创建数据库和表

在项目根目录下已经创建好了sql文件`schema.sql`，用来创建数据库和表。然后我们先进入当前项目所在目录，然后执行`mysql -u root -p < schema.sql`
